...
import logging
from stacking_module import pws, adaptive_filter, robust_stack, nroot_stack, selective_stack
...
def dtw_dvv(ref, cur, para, maxLag, b, direction):
    ...
    if npts > 2:
        ...
        m0, em0 = linear_regression(tvec.flatten()[indx], stbarTime.flatten()[indx], w.flatten()[indx],
                                    intercept_origin=True)

    else:
        Logger.info('not enough points to estimate dv/v for dtw')
        m0 = 0
        em0 = 0
...
def mwcs_dvv(ref, cur, moving_window_length, slide_step, para, smoothing_half_win=5):
    ...
    if len(indx) > 2:
        ...
        Logger.info('not enough points to estimate dv/v for mwcs')
        m0 = 0
        em0 = 0
...
def wcc_dvv(ref, cur, moving_window_length, slide_step, para):
    ...
    if count > 2:
        ...
        Logger.info('not enough points to estimate dv/v for wcc')
        m0 = 0
        em0 = 0
...
def wxs_dvv(ref, cur, allfreq, para, dj=1 / 12, s0=-1, J=-1, sig=False, wvn='morlet', unwrapflag=False):
    ...
    if not allfreq:
        ...
        for it in range(npts):
            ...
            delta_t_m[it], delta_t_unc[it] = linear_regression(freq[freq_indin] * 2 * np.pi,
                                                               phase[freq_indin, itvec[it]], w)
        ...
        if len(w2) > 2:
            ...
            Logger.info('not enough points to estimate dv/v for wts')
            dvv, err = np.nan, np.nan
...
def wtdtw_dvv(ref, cur, allfreq, para, maxLag, b, direction, dj=1 / 12, s0=-1, J=-1, wvn='morlet', normalize=True):
    ...
    # directly take advantage of the real-valued parts of wavelet transforms
        ...
        #                    np.zeros(nfreq, dtype=np.float32), np.zeros(nfreq, dtype=np.float32) # TODO: Add cc and cdp code
...
def wct_modified(y1, y2, dt, dj=1 / 12, s0=-1, J=-1, sig=True, significance_level=0.95, wavelet='morlet',
...
def change_substack_length(timestamp, ndata, stacklen_new, step, dt, stack_method="linear"):
    ...
    timestamp2 = np.array([timestamp_start + k * step for k in range(num_splits) if timestamp_start + k * step < timestamp[-1]])
    #ikeep = np.where(timestamp2 <= timestamp[-1])[0]
    #timestamp2 = timestamp2[ikeep]
